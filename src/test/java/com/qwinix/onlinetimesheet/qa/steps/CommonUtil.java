...
public class CommonUtil {
	...
	public int getRowNumber(String filePath, String sheetName, String caseName) throws IOException {
		int rowNum=0;
		...
		for (Row row: sheet) {
			for(Cell cell: row) {
				...
				if(caseName.equalsIgnoreCase(cellValue)) {
					rowNum = cell.getRowIndex();
					System.out.print("Index = "+ rowNum + "\t");
				}
			}
		}
		return rowNum;
	...
	public String getCellData(String filePath, String sheetName, int rowNumber, String columnName) throws IOException {
		String cellData = "";
		int colNum = -1;
		File src = new File(filePath);
		FileInputStream fis = new FileInputStream(src);
		...
		cellData= sheet.getRow(rowNumber).getCell(colNum).getStringCellValue();
		System.out.println("cellvalues = " + cellData + "\t");
